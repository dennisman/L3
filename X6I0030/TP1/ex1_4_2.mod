...
# d'un tableau de variables indicées sur les Creneaux
	...
	var infSup{C} >= 0 integer;
	var pasSup{C} >= 0 integer;

# Fonction objectif minimizer le nombre d'infirmiere qui font des Hsup

	minimize nbInfsup : sum{j in C} infSup[j];

# Contraintes

	s.t. NbMinParCren{i in C} :  pasSup[i] + pasSup[((i-2) mod 12) +1] + pasSup[((i-4) mod 12)+1] + pasSup[((i-5) mod 12) +1] + infSup[i] + infSup[((i-2) mod 12) +1] + infSup[((i-4) mod 12)+1] + infSup[((i-5) mod 12) +1] + infSup[((i-6) mod 12) +1]  >= mdroite[i];
	
	s.t. NbMaxInf : sum{j in C} (infSup[j]+pasSup[j]) <= nbMaxInf;

# Résolution (qui est ajoutée en fin de fichier si on ne le précise pas)
	solve;

# Affichage des résultats
	display : infSup;	# affichage "standard" des variables
	display : pasSup;
	display : 'z=',sum{j in C} infSup[j]; # affichage de la valeur optimale
