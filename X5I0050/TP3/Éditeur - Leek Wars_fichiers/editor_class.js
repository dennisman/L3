 ...
 * Classe editor
 */
var Editor = function(id, name, valid, level, cores, code) {
	...
	this.editorDiv = $('#editors .editor').last();
	this.editorDiv.hide();
	...
	this.show = function() {
		...
		// Level et cores
		$('#comp-level').text(this.level);
		$('#comp-cores').text(this.cores);
		...
		if (!this.loaded && this.id > 0) {
			...
			_setLocalStorage("lastCode", this.id);
		}
	...
	this.save = function() {
		...
		ajax('editor_update', {id: saveID, compile: true, code: content}, function(data) {
			...
			if (data.length == 0 || error) {
				...
				$('#results').append("<div class='error'>× <i>Erreur du serveur</i></div>");
				return;
			}
			...
			for (var r in data) {
				...
				if (code == 2) {
					...
					$('#results').append("<div class='good'>✓ <b>" + iaName + "</b> valide</div><br>");
					...
					iaEditor.cores = res[2];
					if (ia == current) {
						$('#comp-level').text(res[3]);
						$('#comp-cores').text(res[2]);
					}
					...
					$('#results').append("<div class='error'>× <b>" + iaName + "</b> ligne " + line + "&nbsp; ▶ " + info + "</div><br>");
	...
	this.updateName = function(name) {
		...
		_log("Update infos id " + id + "...");
		
		ajax('editor_update', {id: this.id, save: true, name: name, color: 0});
	}
		...
		if (userChange && _autoClosing) {
			...
			if (changes.from.ch == changes.to.ch) {
				...
				if (pos2 > -1 && nextChar == changes.text[0]) {
					...
					editor.replaceSelection(add[pos]);
		...
		for (i = 0; i < end - start + 1; i++) {
			...
			if (cuttedLine.length > 0) {
				...
				if (line.charAt(pos) == "/" && line.charAt(pos + 1) == "/") {
					this.editor.setLine(start + i, line.slice(0, pos) + line.slice(pos + 2)); // Dé-Comment
				} else {
					this.editor.setLine(start + i, line.slice(0, pos) + "//" + cuttedLine); // Comment
				}
			}
		}
		this.editor.matchBrackets();
	...
	this.duplicateLine = function() {
		...
		if (this.editor.somethingSelected()) {
			...
			this.editor.setLine(start, line + "\n" + line);
		}
	...
	this.mousemove = function(e) {
		...
		if (this.hintDialog.is(':visible')) return;
		...
		if (tokenString != this.hoverToken) {
			...
			for (var i in LEEK_KEYWORDS) {

				if (LEEK_KEYWORDS[i][0] == tokenString) {

					if (LEEK_KEYWORDS[i][3] == 'function') {
						
						var line = editor.editor.getLine(editorPos.line);
						var after = line.substring(editorPos.ch);

						var start;
						var par = 0;
						for (var j = editorPos.ch; j < line.length; ++j) {
							var c = line[j];
						...
						if (argCount == LEEK_KEYWORDS[i][4]) {
							completion = LEEK_KEYWORDS[i]; break;
						}
			...
			if (completion != null) {
				...
				this.detailTimer = setTimeout(function() {
					...
					editor.detailDialog.html(LEEK_KEYWORDS[i][2]);
	...
	this.autocomplete = function(force) {
		...
		function maybeAdd(data) {
			if (typeof data == 'string') {
				if (data.toLowerCase().indexOf(start.toLowerCase()) == 0) {
					completions.push({text: data, details: "Mot clé " + data, type: 'keyword'});
				}
			} else {
		...
		for (var v = token.state.localVars; v; v = v.next) {
			if (v.name.toLowerCase().indexOf(start.toLowerCase()) == 0) {
				completions.push({text: v.name, name: v.name, details: "Variable <b>" + v.name + "</b>", type: 'keyword'});
			}
		}
		...
		for (var i in this.getGlobalVars(vars)) {
			...
			for (var v = file; v; v = v.next) {
				if (v.name.toLowerCase().indexOf(start.toLowerCase()) == 0) {
					...
					if (i != this.id) text += "<br><br>Variable définie dans l'IA <b>" + editors[i].name + "</b>.";
		...
		// Ajout des fonctions
		forEach(LEEK_KEYWORDS, maybeAdd);
		...
		this.completionFrom = {line: cur.line, ch: token.start};
	...
	this.getGlobalVars = function(vars) {
		...
		_log("getGlobalVars de " + this.name);
