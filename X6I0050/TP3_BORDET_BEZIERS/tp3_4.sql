...
CREATE OR REPLACE TRIGGER avis_before_ins_row_upd
       BEFORE INSERT OR UPDATE
       ON Avis
       FOR EACH ROW
       DECLARE
       nb NUMBER;
       refLivreAjoute %Livres.refl;
       pasAchat exception 
       BEGIN 
       dbms_output.put_line('Ta le seum');
       SELECT count(*) INTO nb FROM Achats WHERE Achats.idcl =new.idcl and Achats.refl = new.refl;
       dbms_ouput.put_line(nb);
       IF (nb = 0)  THEN
       	  RAISE pasAchat;
       END IF;
       EXCEPTION 
       WHEN pasAchat then RAISE_APPLICATION_ERROR(-20002, 'il a pas de livre');
END;
/
