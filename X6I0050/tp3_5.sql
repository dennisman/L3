...
DECLARE 
	prefl Livres.refl %type ;
	pidcl Clients.idcl %type ;
	nb NUMBER;
	pnote Avis.note %type ;
	pavis Avis.commentaire %type ;
	pas_modif EXCEPTION
BEGIN
	prefl := &prefl;
	pidcl := &pidcl;
	select count (*) into nb from Avis where refl = prefl and idcl = pidcl ;
	IF (nb < 0) THEN
	   RAISE pas_modif;
	ELSE 
	   pnote := &pnote;
	   pAvis := &pAvis;
	   update Avis set note =: pnote and commentaire =: pavis where idcl = pidcl and refl = prefl;
	END IF;

EXCEPTION
	when pas_modif then
	RAISE_APPLICATION_ERROR(-20001, 'Cest pas ton avis');
